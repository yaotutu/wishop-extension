...
订单云同步用于把插件自己产生、微信小店 API 无法重新获取的数据同步到云端。微信小店 API 能获取的数据不进入订单云同步范围，避免重复存储订单业务数据和扩大隐私风险。
...
订单同步只包含微信小店 API 拿不到、但插件订单管理功能需要跨设备保留的数据：
...
- 用户开启订单同步后，立即同步一次。
- 打开后台管理页或切换店铺时，同步一次。
- 保存货源、保存采购单、淘宝工作页读取采购单完成后，延迟几秒同步一次。
...
订单云同步接口收到的 payload 不应包含上述字段。服务端仍应做字段校验，拒绝明显包含真实地址或微信订单详情的大型 payload。
