...
export async function setProductSources(accountId: string, productId: string, sources: Array<Pick<ProductSourceItem, 'id' | 'url' | 'quantity' | 'remark' | 'createdAt' | 'updatedAt'>>): Promise<ProductSourceBinding> {
  const now = Date.now();
